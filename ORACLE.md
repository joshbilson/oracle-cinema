...
Ultra's qBittorrent keeps ratio, total-seeding-time, and inactive-seeding-time auto-stops disabled. Queue limits are three concurrent downloads, 200 uploads, and 500 total torrents; slow torrents do not consume a queue slot. The `OracleCinemaSeedboxRetention` Windows service checks the Ultra user quota hourly and leaves every seed untouched below 75% usage. Its status is `C:/media/meta/oracle-cinema/seedbox-retention-status.json`.

At 75% quota use, it considers only managed TorrentLeech Radarr/Sonarr torrents with ratio at least 1, 45 active seed days, and 30 days without data activity. Higher pressure gradually relaxes the age and idle limits, never below 14 active seed days and 7 idle days at 95% use. An uploading torrent, a torrent with reported leechers, an incomplete or queued torrent, a seed without a working TorrentLeech tracker, an unrecognized tracker/category, or one tagged `keep`, `retain`, `pinned`, or `no-prune` is never retired. These limits exceed [TorrentLeech's 10-day Registered minimum](https://wiki.torrentleech.me/doku.php/hnr), including for freeleech, and leave time for tracker accounting.

Before retirement, Oracle must have completed a recent rsync pass and every imported media file must have a matching size and SHA-256 digest on its local archive. Only then does the service remove the torrent's seedbox download and its imported hardlink. It retires at most five torrents per hour. At 90% quota use, if cleanup cannot free enough space, it holds new downloads by setting qBittorrent's active download limit to zero and disabling the slow-torrent queue exemption; it keeps existing seeds online and restores the three-download limit below 80% usage. Private/uncategorized torrents are excluded. The source is in `scripts/oracle/seedbox_retention.py` and `scripts/oracle/ultra_retention_worker.py`.
